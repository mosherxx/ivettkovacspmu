...
export function cookie(token:string,request:Request,clear=false){return `${SESSION_COOKIE}=${token}; Path=/; HttpOnly; SameSite=Strict; Max-Age=${clear?0:28800}${new URL(request.url).protocol==='https:'?'; Secure':''}`;}
